...
namespace BackendDotnet.Controllers
{
    ...
    public class UploadController : ControllerBase
    {
        ...
        {
            if (file == null) return BadRequest(new { detail = "No file uploaded" });
            ...
            try
            {
                if (ext == ".xlsx" || ext == ".xls" || ext == ".xlsm")
                {
                    ...
                    foreach (var worksheet in wb.Worksheets)
                    {
                        ...
                        if (used == null)
                        {
                            sheetResults.Add(new { name = worksheet.Name, headers = Array.Empty<string>(), rows = new List<List<string>>(), csv = string.Empty });
                            continue;
                        }
                        ...
                        bool CellHasValue(string? v) => !(v == null || string.IsNullOrWhiteSpace(v));
                        bool RowIsEmpty(List<string?> row) => row.All(c => !CellHasValue(c));
                        ...
                        // generate CSV string for this sheet
                        string csvString;
                        ...
                        sheetResults.Add(new { name = worksheet.Name, headers = headerRow, rows = dataRows, csv = csvString });
                    }
                ...
                else
                {
                    ...
                    bool CsvCellHasValue(string v) => !(v == null || string.IsNullOrWhiteSpace(v));
                    bool CsvRowIsEmpty(List<string> row) => row.All(c => !CsvCellHasValue(c));

                    var filtered = allRows.Where(r => !CsvRowIsEmpty(r)).ToList();
                    if (!filtered.Any())
                    {
                        parsed = new { type = "csv_or_text", headers = Array.Empty<string>(), rows = new List<List<string>>() };
                    }
                    else
                    {
